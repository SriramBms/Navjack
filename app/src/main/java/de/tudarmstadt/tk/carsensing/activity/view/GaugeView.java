...
public class GaugeView extends View {
    ...
    public static final float INNER_TEXT_GAUGE_WIDTH = 0.03f;
    

    //public static final int[] OUTER_SHADOW_COLORS = {Color.argb(40, 255, 254, 187), Color.argb(20, 255, 247, 219),
           // Color.argb(5, 255, 255, 255)};
            
    public static final int[] OUTER_SHADOW_COLORS = {Color.argb(256, 256, 36, 0), Color.argb(220, 256, 36, 0),
                Color.argb(10, 128, 00, 00)};
   ...
   // public static final float[] RANGE_VALUES = {16.0f, 25.0f, 40.0f, 100.0f};
    ...
    public static final float[] RANGE_VALUES = {24.0f, 48.0f, 60.0f};
    public static final int[] RANGE_COLORS = {Color.rgb(27, 202, 33), Color.rgb(232, 231, 33), Color.rgb(232, 111, 33)};
    ...
    public void initDrawingRects() {
      ...
      mTextGaugeRect = new RectF(mOuterRimRect.left, mOuterRimRect.bottom+0.05f,mOuterRimRect.right,mOuterRimRect.bottom+0.4f);
      mBorderDarkRect = new RectF(mTextGaugeRect.left+INNER_TEXT_GAUGE_WIDTH,mTextGaugeRect.top+INNER_TEXT_GAUGE_WIDTH,mTextGaugeRect.right-INNER_TEXT_GAUGE_WIDTH,mTextGaugeRect.bottom-INNER_TEXT_GAUGE_WIDTH);
      ...
      //mTextDisplayRect = new RectF(mBorderDarkRect.left+INNER_RIM_BORDER_WIDTH,mBorderDarkRect.top+INNER_RIM_BORDER_WIDTH,mBorderDarkRect.right - INNER_RIM_BORDER_WIDTH, mBorderDarkRect.bottom - INNER_RIM_BORDER_WIDTH); 
    ...
    private void initDrawingTools() {
        ...
        if (mShowOuterRim) {
            mOuterRimPaint = getDefaultOuterRimPaint(0);
        }
        if (mShowInnerRim) {
        ...
        mTextGaugePaint = getDefaultOuterRimPaint(1);
        mTextDisplayPaint = getDefaultTextDisplayPaint();
    ...
    private Paint getDefaultTextDisplayPaint() {
		...
		int[] colorrange = new int[]{Color.rgb(30, 30, 30),Color.rgb(50, 50, 50),Color.rgb(10, 10, 10),Color.rgb(10, 10, 10)};
		float[] colorpos = new float[]{0.0f,0.5f,0.5f,1.0f};
		
		
		/*final LinearGradient reflectionGradient = new LinearGradient(mTextDisplayRect.left, mTextDisplayRect.top, mTextGaugeRect.centerX(),
				mTextGaugeRect.centerY(), Color.rgb(30, 30, 30), Color.rgb(80, 80, 80), TileMode.REPEAT);*/
		...
		paint.setShader(new ComposeShader(reflectionGradient,reflectionGradientUniform,PorterDuff.Mode.SCREEN));
    ...
    private Paint getDefaultOuterBorderPaint() {
    	final Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.brushed_alu2);
    	final BitmapShader darkMetalShader = new BitmapShader(bitmap, TileMode.REPEAT, TileMode.REPEAT);
    	final Matrix matrix = new Matrix();
        matrix.setScale(1.0f / bitmap.getWidth(), 1.0f / bitmap.getHeight());
        darkMetalShader.setLocalMatrix(matrix);
        final RadialGradient radialGradient = new RadialGradient(0.5f, 0.5f, mFaceRect.width() / 2, new int[]{Color.rgb(100,100,100), Color.rgb(200,200,200)}, new float[]{0.50f, 0.99f}, TileMode.MIRROR);
        final Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        //paint.setStyle(Paint.Style.FILL);
        //paint.setColor(Color.argb(245, 100, 100, 100));
        paint.setShader(new ComposeShader(darkMetalShader,radialGradient, PorterDuff.Mode.MULTIPLY));
        paint.setFilterBitmap(true);
        return paint;
    }

    public Paint getDefaultOuterRimPaint(int GaugeType) {
        // Use a linear gradient to create the 3D effect
    	LinearGradient verticalGradient;
    	if(GaugeType == 0){
    		verticalGradient = new LinearGradient(mOuterRimRect.left, mOuterRimRect.top, mOuterRimRect.right,
                mOuterRimRect.bottom, Color.rgb(255, 255, 255), Color.rgb(84, 90, 100), TileMode.REPEAT);
    	}else{
    		verticalGradient = new LinearGradient(mTextGaugeRect.left, mTextGaugeRect.top, mTextGaugeRect.right,
    				mTextGaugeRect.bottom, Color.rgb(255, 255, 255), Color.rgb(84, 90, 100), TileMode.REPEAT);
    	}
    ...
    public void setDefaultScaleRangePaints() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            mRangePaints[i].setTextSize(0.07f);
            mRangePaints[i].setTypeface(Typeface.MONOSPACE);
    ...
    public Paint getDefaultTextValuePaint() {
        ...
        paint.setTypeface(FontUtil.getDigiFont(getContext(), 0));
        int[] colorrange = new int[]{Color.rgb(250, 250, 250),Color.rgb(220, 220, 220),Color.rgb(250, 250, 250),Color.rgb(250, 250, 250)};
		float[] colorpos = new float[]{0.0f,0.5f,0.5f,1.0f};
        final LinearGradient reflectionGradient = new LinearGradient(mTextDisplayRect.left, mTextDisplayRect.top, mTextGaugeRect.right,
				mTextGaugeRect.bottom, colorrange,colorpos, TileMode.CLAMP);
    ...
    private void drawGauge() {
        ...
        //Log.e("GaugeView",getWidth()+":"+getHeight());
    ...
    private void drawRim(final Canvas canvas) {
       ...
       canvas.drawRoundRect(mTextGaugeRect, 0.05f, 0.05f, mTextGaugePaint);
       canvas.drawRoundRect(mBorderDarkRect, 0.05f, 0.05f, mInnerRimBorderDarkPaint);
       canvas.drawRoundRect(mBorderLightRect, 0.05f, 0.05f, mInnerRimBorderLightPaint);
       canvas.drawRoundRect(mTextDisplayRect, 0.05f, 0.05f, mTextDisplayPaint);
    }
    ...
    private void drawText(final Canvas canvas) {
        ...
        final float startY = CENTER + 0.78f;
        textValue = Integer.parseInt(textValue)+" RPM";
        
        canvas.drawText(textValue, startX, startY, mTextValuePaint);
        //canvas.drawText("TEST", startX, startY, mTextValuePaint);

        if (!TextUtils.isEmpty(mTextUnit)) {
            canvas.drawText(mTextUnit, CENTER + textValueWidth / 2, CENTER, mTextUnitPaint);
        }
    }
    ...
    private float getValueForTick(final int tick) {
        ...
        return Float.parseFloat(String.format("%.2f", (tick * (mDivisionValue / mSubdivisions))));
    }
